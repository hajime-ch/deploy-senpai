...
func (d *Deployer) StartDeployment(appName, branch, imageTag string) (*Deployment, error) {
	...
	deployment := &Deployment{
		...
		URL:           fmt.Sprintf("https://%s.%s", subdomain, d.cfg.BaseDomainForApp(appName)),
		ImageTag:      imageTag,
		CreatedAt:     now,
		UpdatedAt:     now,
		LastActivity:  now,
...
func (d *Deployer) Deploy(ctx context.Context, appName, branch, imageTag string) (*Deployment, error) {
	...
	templateData := TemplateData{
		...
		BaseDomain:      d.cfg.BaseDomainForApp(appName),
		Subdomain:       subdomain,
		...
		for _, branch := range branches {
			...
			if metadata != nil {
				...
				d.deployments[key] = &Deployment{
					...
					URL:           fmt.Sprintf("https://%s.%s", subdomain, d.cfg.BaseDomainForApp(app.Name())),
